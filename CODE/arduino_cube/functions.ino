...
void XY_to_threeWay(float pwm_X, float pwm_Y) {
  ...
  Motor1_control(-m1); //-m1
  Motor2_control(-m2); //-m2
  Motor3_control(m3);  //m3
}
...
int Tuning() {
  if  (!digitalRead(CALIBRATE) && calibrated == false) { // if press button during power on reset calibration
    if (!calibrating) {
        calibrating = true;
